    ...
    try {
      ...
      if (problemsArray.length > 0) {
        ...
        if (firstProblem.supportedLanguages && firstProblem.supportedLanguages.length > 0 && !firstProblem.supportedLanguages.includes(langToUse)) {
          ...
          const langRes = await axios.get(`/problems/round/2?language=${langToUse}`);
          ...
          setCode(savedCode || langProblemsArray[0].starterCode || '');
        } else {
    ...
    if (selectedProblem) {
      if (selectedProblem.supportedLanguages && selectedProblem.supportedLanguages.length > 0 && !selectedProblem.supportedLanguages.includes(selectedLanguage)) {
        ...
        try {
          ...
          setCode(savedCode || newProblem.starterCode || '');
          setMistakes(0);
    ...
    try {
      ...
      if (status === 'Accepted') {
        ...
        if (allSolved) {
          ...
          navigate('/round-complete', { state: { timeTaken: elapsed, mistakes: totalMistakes, round: 2 } });
        } else {
      ...
      <div className="round-content-wrapper">
        ...
        <div className="language-selector">
          ...
          <select id="language" value={selectedLanguage} onChange={handleLanguageChange}>
            {LANGUAGES.filter(lang => !problem?.supportedLanguages || problem.supportedLanguages.includes(lang.id)).map(lang => (<option key={lang.id} value={lang.id}>{lang.name}</option>))}
          </select>
        </div>
